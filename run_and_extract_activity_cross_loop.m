...
pairstrength_values = 0:0.05:0.2;

for pairstrength = pairstrength_values
    %% Create directory for current incor value
    directory_name = sprintf('sims/sims_%s_%i_cross_paircorr_%.2f', str_now, T, pairstrength);

    [~, ~, ~] = mkdir(directory_name);

    parfor xx = 1:REPS
        fprintf('Get all parameters for sim %i with cross %.2f.\n', xx, pairstrength)
        ...
        fprintf('Starting sim %i with cross %.2f\n', xx, pairstrength)
        ...
        fprintf('Finished sim %i with cross %.2f\n', xx, pairstrength)
    end

    fprintf('Finished simulating batch for cross %.2f\n', pairstrength)

    %%
    fprintf('Now extracting data for cross %.2f\n', pairstrength)
