function [accLengths, accSyns, accDens, tot_tree_length_time, dend_add_time, dend_remove_time, connect_syn_time, syn_add_time, syn_remove_time, sel, stab] = process_simulations_moving_syn(load_dir_str, REPS, N, T)
    ...
    parfor i = 1:REPS
        ...
        GroupsVid = load([load_dir_str, '/sim_', num2str(i), '/GroupsVid.mat']).GroupsVid;
        GroupID = mats.GroupID;
        % sel(i,:) = selectivityvstime_trad(SimVid, Syn, GroupID);
        sel(i,:) = selectivityvstime_moving_new(SimVid, GroupsVid, GroupID);
